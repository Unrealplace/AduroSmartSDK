...
@class AduroGCDAsyncUdpSendPacket;

NSString *const AduroGCDAsyncUdpSocketException = @"GCDAsyncUdpSocketException";
NSString *const AduroGCDAsyncUdpSocketErrorDomain = @"GCDAsyncUdpSocketErrorDomain";

NSString *const AduroGCDAsyncUdpSocketQueueName = @"GCDAsyncUdpSocket";
NSString *const AduroGCDAsyncUdpSocketThreadName = @"GCDAsyncUdpSocket-CFStream";
...
@interface AduroGCDAsyncUdpSocket ()
{
	...
	AduroGCDAsyncUdpSendPacket *currentSend;
...
**/
@interface AduroGCDAsyncUdpSendPacket : NSObject {
@public
	NSData *buffer;
...
@implementation AduroGCDAsyncUdpSendPacket
...
@interface AduroGCDAsyncUdpSpecialPacket : NSObject {
@public
//	uint8_t type;
...
@implementation AduroGCDAsyncUdpSpecialPacket
...
@implementation AduroGCDAsyncUdpSocket
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	...
	if ((self = [super init]))
	{
		...
		max4ReceiveSize = 15216;
		max6ReceiveSize = 15216;
		...
		{
			socketQueue = dispatch_queue_create([AduroGCDAsyncUdpSocketQueueName UTF8String], NULL);
		}
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:AduroGCDAsyncUdpSocketErrorDomain
...
- (NSError *)socketClosedError
{
	...
	return [NSError errorWithDomain:AduroGCDAsyncUdpSocketErrorDomain code:GCDAsyncUdpSocketClosedError userInfo:userInfo];
}
...
- (BOOL)connectToHost:(NSString *)host onPort:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		AduroGCDAsyncUdpSpecialPacket *packet = [[AduroGCDAsyncUdpSpecialPacket alloc] init];
...
- (void)maybeConnect
{
	...
	BOOL sendQueueReady = [currentSend isKindOfClass:[AduroGCDAsyncUdpSpecialPacket class]];
	
	if (sendQueueReady)
	{
		AduroGCDAsyncUdpSpecialPacket *connectPacket = (AduroGCDAsyncUdpSpecialPacket *)currentSend;
...
- (void)sendData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	AduroGCDAsyncUdpSendPacket *packet = [[AduroGCDAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	AduroGCDAsyncUdpSendPacket *packet = [[AduroGCDAsyncUdpSendPacket alloc] initWithData:data timeout:timeout tag:tag];
	packet->addressFamily = [AduroGCDAsyncUdpSocket familyFromAddress:remoteAddr];
...
- (void)maybeDequeueSend
{
	...
	if (currentSend == nil)
	{
		...
		while ([sendQueue count] > 0)
		{
			...
			if ([currentSend isKindOfClass:[AduroGCDAsyncUdpSpecialPacket class]])
			{
				[self maybeConnect];
...
- (void)doPreSend
{
	...
	if (sendFilterBlock && sendFilterQueue)
	{
		...
		if (sendFilterAsync)
		{
			...
			AduroGCDAsyncUdpSendPacket *sendPacket = currentSend;
...
+ (void)listenerThread
{
	@autoreleasepool {
		...
		[[NSThread currentThread] setName:AduroGCDAsyncUdpSocketThreadName];
...
+ (void)addStreamListener:(AduroGCDAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
...
+ (void)removeStreamListener:(AduroGCDAsyncUdpSocket *)asyncUdpSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == listenerThread, @"Invoked on wrong thread");
...
{
	@autoreleasepool {
		AduroGCDAsyncUdpSocket *asyncUdpSocket = (__bridge AduroGCDAsyncUdpSocket *)pInfo;
