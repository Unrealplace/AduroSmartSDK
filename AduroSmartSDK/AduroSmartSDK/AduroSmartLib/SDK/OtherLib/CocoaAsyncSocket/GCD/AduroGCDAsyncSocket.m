...
NSString *const AduroGCDAsyncSocketException = @"GCDAsyncSocketException";
NSString *const AduroGCDAsyncSocketErrorDomain = @"GCDAsyncSocketErrorDomain";

NSString *const AduroGCDAsyncSocketQueueName = @"GCDAsyncSocket";
NSString *const AduroGCDAsyncSocketThreadName = @"GCDAsyncSocket-CFStream";

#if SECURE_TRANSPORT_MAYBE_AVAILABLE
NSString *const AduroGCDAsyncSocketSSLCipherSuites = @"GCDAsyncSocketSSLCipherSuites";
#if TARGET_OS_IPHONE
NSString *const AduroGCDAsyncSocketSSLProtocolVersionMin = @"GCDAsyncSocketSSLProtocolVersionMin";
NSString *const AduroGCDAsyncSocketSSLProtocolVersionMax = @"GCDAsyncSocketSSLProtocolVersionMax";
#else
NSString *const AduroGCDAsyncSocketSSLDiffieHellmanParameters = @"GCDAsyncSocketSSLDiffieHellmanParameters";
#endif
#endif
...
@interface AduroGCDAsyncSocketPreBuffer : NSObject
{
	uint8_t *preBuffer;
...
@implementation AduroGCDAsyncSocketPreBuffer
...
**/
@interface AduroGCDAsyncReadPacket : NSObject
{
  @public
...
- (NSUInteger)readLengthForTermWithPreBuffer:(AduroGCDAsyncSocketPreBuffer *)preBuffer found:(BOOL *)foundPtr;

- (NSInteger)searchForTermAfterPreBuffering:(ssize_t)numBytes;

@end

@implementation AduroGCDAsyncReadPacket
...
**/
@interface AduroGCDAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation AduroGCDAsyncWritePacket
...
**/
@interface AduroGCDAsyncSpecialPacket : NSObject
{
  @public
...
@implementation AduroGCDAsyncSpecialPacket
...
@implementation AduroGCDAsyncSocket
{
	uint32_t flags;
	uint16_t config;
	...
	AduroGCDAsyncReadPacket *currentRead;
	AduroGCDAsyncWritePacket *currentWrite;
	
	unsigned long socketFDBytesAvailable;
	
	AduroGCDAsyncSocketPreBuffer *preBuffer;
	...
	AduroGCDAsyncSocketPreBuffer *sslPreBuffer;
...
- (id)initWithDelegate:(id)aDelegate delegateQueue:(dispatch_queue_t)dq socketQueue:(dispatch_queue_t)sq
{
	if((self = [super init]))
	{
		...
		{
			socketQueue = dispatch_queue_create([AduroGCDAsyncSocketQueueName UTF8String], NULL);
		}
		...
		preBuffer = [[AduroGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
	}
	return self;
}
...
- (BOOL)acceptOnInterface:(NSString *)inInterface port:(uint16_t)port error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		if (enableIPv4)
		{
			...
			__weak AduroGCDAsyncSocket *weakSelf = self;
			
			dispatch_source_set_event_handler(accept4Source, ^{ @autoreleasepool {
			#pragma clang diagnostic push
			#pragma clang diagnostic warning "-Wimplicit-retain-self"
				
				__strong AduroGCDAsyncSocket *strongSelf = weakSelf;
...
- (BOOL)doAccept:(int)parentSocketFD
{
	...
	if (delegateQueue)
	{
		...
		dispatch_async(delegateQueue, ^{ @autoreleasepool {
			...
			AduroGCDAsyncSocket *acceptedSocket = [[AduroGCDAsyncSocket alloc] initWithDelegate:theDelegate
                ...
                error:(NSError **)errPtr
{
	...
	dispatch_block_t block = ^{ @autoreleasepool {
		...
		#pragma clang diagnostic warning "-Wimplicit-retain-self"
			...
			NSMutableArray *addresses = [AduroGCDAsyncSocket lookupHost:hostCpy port:port error:&lookupErr];
			...
			else
			{
				...
				{
					if (!address4 && [AduroGCDAsyncSocket isIPv4Address:address])
					{
						address4 = address;
					}
					else if (!address6 && [AduroGCDAsyncSocket isIPv6Address:address])
					{
						address6 = address;
					}
				}
...
- (NSError *)badConfigError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:AduroGCDAsyncSocketErrorDomain code:GCDAsyncSocketBadConfigError userInfo:userInfo];
}
...
- (NSError *)badParamError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:AduroGCDAsyncSocketErrorDomain code:GCDAsyncSocketBadParamError userInfo:userInfo];
}
...
- (NSError *)connectTimeoutError
{
	...
	return [NSError errorWithDomain:AduroGCDAsyncSocketErrorDomain code:GCDAsyncSocketConnectTimeoutError userInfo:userInfo];
}
...
- (NSError *)readMaxedOutError
{
	...
	return [NSError errorWithDomain:AduroGCDAsyncSocketErrorDomain code:GCDAsyncSocketReadMaxedOutError userInfo:info];
}
...
- (NSError *)readTimeoutError
{
	...
	return [NSError errorWithDomain:AduroGCDAsyncSocketErrorDomain code:GCDAsyncSocketReadTimeoutError userInfo:userInfo];
}
...
- (NSError *)writeTimeoutError
{
	...
	return [NSError errorWithDomain:AduroGCDAsyncSocketErrorDomain code:GCDAsyncSocketWriteTimeoutError userInfo:userInfo];
}
...
- (NSError *)connectionClosedError
{
	...
	return [NSError errorWithDomain:AduroGCDAsyncSocketErrorDomain code:GCDAsyncSocketClosedError userInfo:userInfo];
}
...
- (NSError *)otherError:(NSString *)errMsg
{
	...
	return [NSError errorWithDomain:AduroGCDAsyncSocketErrorDomain code:GCDAsyncSocketOtherError userInfo:userInfo];
}
                        ...
                        tag:(long)tag
{
	...
	AduroGCDAsyncReadPacket *packet = [[AduroGCDAsyncReadPacket alloc] initWithData:buffer
	                                                          startOffset:offset
	                                                            maxLength:length
	                                                              timeout:timeout
	                                                           readLength:0
...
- (float)progressOfReadReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentRead || ![currentRead isKindOfClass:[AduroGCDAsyncReadPacket class]])
		{
...
- (void)maybeDequeueRead
{
	...
	if ((currentRead == nil) && (flags & kConnected))
	{
		if ([readQueue count] > 0)
		{
			...
			if ([currentRead isKindOfClass:[AduroGCDAsyncSpecialPacket class]])
			{
...
- (void)completeCurrentRead
{
	...
	{
		AduroGCDAsyncReadPacket *theRead = currentRead; // Ensure currentRead retained since result may not own buffer
...
- (void)doReadTimeout
{
	...
	{
		AduroGCDAsyncReadPacket *theRead = currentRead;
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	AduroGCDAsyncWritePacket *packet = [[AduroGCDAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (float)progressOfWriteReturningTag:(long *)tagPtr bytesDone:(NSUInteger *)donePtr total:(NSUInteger *)totalPtr
{
	...
	dispatch_block_t block = ^{
		...
		if (!currentWrite || ![currentWrite isKindOfClass:[AduroGCDAsyncWritePacket class]])
		{
...
- (void)maybeDequeueWrite
{
	...
	if ((currentWrite == nil) && (flags & kConnected))
	{
		if ([writeQueue count] > 0)
		{
			...
			if ([currentWrite isKindOfClass:[AduroGCDAsyncSpecialPacket class]])
			{
				LogVerbose(@"Dequeued GCDAsyncSpecialPacket");
...
- (void)doWriteTimeout
{
	...
	{
		AduroGCDAsyncWritePacket *theWrite = currentWrite;
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	AduroGCDAsyncSpecialPacket *packet = [[AduroGCDAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	if ((flags & kStartingReadTLS) && (flags & kStartingWriteTLS))
	{
		...
		#if TARGET_OS_IPHONE
		{
			AduroGCDAsyncSpecialPacket *tlsPacket = (AduroGCDAsyncSpecialPacket *)currentRead;
...
{
	AduroGCDAsyncSocket *asyncSocket = (__bridge AduroGCDAsyncSocket *)connection;
...
- (void)ssl_startTLS
{
	...
	#if TARGET_OS_IPHONE
	{
		...
		NSString *sslMinLevel = [tlsSettings objectForKey:AduroGCDAsyncSocketSSLProtocolVersionMin];
		NSString *sslMaxLevel = [tlsSettings objectForKey:AduroGCDAsyncSocketSSLProtocolVersionMax];
	...
	value = [tlsSettings objectForKey:AduroGCDAsyncSocketSSLCipherSuites];
	if (value)
	{
	...
	sslPreBuffer = [[AduroGCDAsyncSocketPreBuffer alloc] initWithCapacity:(1024 * 4)];
...
- (void)cf_startTLS
{
	...
	NSAssert([currentRead isKindOfClass:[AduroGCDAsyncSpecialPacket class]], @"Invalid read packet for startTLS");
	NSAssert([currentWrite isKindOfClass:[AduroGCDAsyncSpecialPacket class]], @"Invalid write packet for startTLS");
...
{
	[[NSThread currentThread] setName:AduroGCDAsyncSocketThreadName];
...
+ (void)scheduleCFStreams:(AduroGCDAsyncSocket *)asyncSocket
{
	LogTrace();
...
+ (void)unscheduleCFStreams:(AduroGCDAsyncSocket *)asyncSocket
{
	LogTrace();
	NSAssert([NSThread currentThread] == cfstreamThread, @"Invoked on wrong thread");
...
{
	AduroGCDAsyncSocket *asyncSocket = (__bridge AduroGCDAsyncSocket *)pInfo;
