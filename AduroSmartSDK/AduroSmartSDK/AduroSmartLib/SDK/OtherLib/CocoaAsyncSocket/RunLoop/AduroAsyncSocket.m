...
NSString *const AduroAsyncSocketException = @"AsyncSocketException";
NSString *const AduroAsyncSocketErrorDomain = @"AsyncSocketErrorDomain";
...
@interface AduroAsyncSocket (Private)
...
**/
@interface AduroAsyncReadPacket : NSObject
{
  @public
...
@implementation AduroAsyncReadPacket
...
**/
@interface AduroAsyncWritePacket : NSObject
{
  @public
	NSData *buffer;
	NSUInteger bytesDone;
...
@implementation AduroAsyncWritePacket
...
**/
@interface AduroAsyncSpecialPacket : NSObject
{
  @public
...
@implementation AduroAsyncSpecialPacket
...
@implementation AduroAsyncSocket
...
- (void)checkForThreadSafety
{
	if (theRunLoop && (theRunLoop != CFRunLoopGetCurrent()))
	{
		...
		[NSException raise:AduroAsyncSocketException
...
- (float)progressOfReadReturningTag:(long *)tag bytesDone:(NSUInteger *)done total:(NSUInteger *)total
{
	...
	if (!theCurrentRead || ![theCurrentRead isKindOfClass:[AduroAsyncReadPacket class]])
	{
		if (tag != NULL)   *tag = 0;
		if (done != NULL)  *done = 0;
...
- (float)progressOfWriteReturningTag:(long *)tag bytesDone:(NSUInteger *)done total:(NSUInteger *)total
{
	...
	if (!theCurrentWrite || ![theCurrentWrite isKindOfClass:[AduroAsyncWritePacket class]])
	{
		if (tag != NULL)   *tag = 0;
		if (done != NULL)  *done = 0;
		if (total != NULL) *total = 0;
...
- (BOOL)createSocketForAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
	...
	else
	{
		if (errPtr)
		{
			...
			*errPtr = [NSError errorWithDomain:AduroAsyncSocketErrorDomain code:AsyncSocketCFSocketError userInfo:info];
		}
		return NO;
	}
...
- (void)doAcceptFromSocket:(CFSocketRef)parentSocket withNewNativeSocket:(CFSocketNativeHandle)newNativeSocket
{
	if(newNativeSocket)
	{
		...
		AduroAsyncSocket *newSocket = [[[self class] alloc] initWithDelegate:theDelegate];
...
- (void)recoverUnreadData
{
	if(theCurrentRead != nil)
	{
		...
		if(([theCurrentRead isKindOfClass:[AduroAsyncReadPacket class]]) && (theCurrentRead->bytesDone > 0))
		{
...
- (NSError *)getSocketError
{
	...
	return [NSError errorWithDomain:AduroAsyncSocketErrorDomain code:AsyncSocketCFSocketError userInfo:info];
}
...
- (NSError *)getAbortError
{
	...
	return [NSError errorWithDomain:AduroAsyncSocketErrorDomain code:AsyncSocketCanceledError userInfo:info];
}
...
- (NSError *)getConnectTimeoutError
{
	...
	return [NSError errorWithDomain:AduroAsyncSocketErrorDomain code:AsyncSocketConnectTimeoutError userInfo:info];
}
...
- (NSError *)getReadMaxedOutError
{
	...
	return [NSError errorWithDomain:AduroAsyncSocketErrorDomain code:AsyncSocketReadMaxedOutError userInfo:info];
}
...
- (NSError *)getReadTimeoutError
{
	...
	return [NSError errorWithDomain:AduroAsyncSocketErrorDomain code:AsyncSocketReadTimeoutError userInfo:info];
}
...
- (NSError *)getWriteTimeoutError
{
	...
	return [NSError errorWithDomain:AduroAsyncSocketErrorDomain code:AsyncSocketWriteTimeoutError userInfo:info];
}
...
- (NSString *)description
{
	...
	if (theCurrentRead == nil || [theCurrentRead isKindOfClass:[AduroAsyncSpecialPacket class]])
	...
	if (theCurrentWrite == nil || [theCurrentWrite isKindOfClass:[AduroAsyncSpecialPacket class]])
                        ...
                        tag:(long)tag
{
	...
	AduroAsyncReadPacket *packet = [[AduroAsyncReadPacket alloc] initWithData:buffer
	                                                    startOffset:offset
	                                                      maxLength:length
	                                                        timeout:timeout
	                                                     readLength:0
...
- (void)maybeDequeueRead
{
	...
	if((theCurrentRead == nil) && (theReadStream != NULL))
	{
		if([theReadQueue count] > 0)
		{
			...
			if([theCurrentRead isKindOfClass:[AduroAsyncSpecialPacket class]])
			{
...
- (void)writeData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	AduroAsyncWritePacket *packet = [[AduroAsyncWritePacket alloc] initWithData:data timeout:timeout tag:tag];
...
- (void)maybeDequeueWrite
{
	...
	if((theCurrentWrite == nil) && (theWriteStream != NULL))
	{
		if([theWriteQueue count] > 0)
		{
			...
			if([theCurrentWrite isKindOfClass:[AduroAsyncSpecialPacket class]])
			{
				// Attempt to start TLS
...
- (void)startTLS:(NSDictionary *)tlsSettings
{
	...
	AduroAsyncSpecialPacket *packet = [[AduroAsyncSpecialPacket alloc] initWithTLSSettings:tlsSettings];
...
- (void)maybeStartTLS
{
	...
	{
		AduroAsyncSpecialPacket *tlsPacket = (AduroAsyncSpecialPacket *)theCurrentRead;
...
static void MyCFSocketCallback (CFSocketRef sref, CFSocketCallBackType type, CFDataRef inAddress, const void *pData, void *pInfo)
{
	@autoreleasepool {
		...
		AduroAsyncSocket *theSocket = (__bridge AduroAsyncSocket *)pInfo;
