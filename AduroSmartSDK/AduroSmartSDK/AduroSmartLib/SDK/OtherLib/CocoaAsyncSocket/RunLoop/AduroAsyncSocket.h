...
@class AduroAsyncSocket;
@class AduroAsyncReadPacket;
@class AduroAsyncWritePacket;

extern NSString *const AduroAsyncSocketException;
extern NSString *const AduroAsyncSocketErrorDomain;
...
**/
- (void)onSocket:(AduroAsyncSocket *)sock willDisconnectWithError:(NSError *)err;
...
**/
- (void)onSocketDidDisconnect:(AduroAsyncSocket *)sock;

/**
 * Called when a socket accepts a connection.  Another socket is spawned to handle it. The new socket will have
 * the same delegate and will call "onSocket:didConnectToHost:port:".
**/
- (void)onSocket:(AduroAsyncSocket *)sock didAcceptNewSocket:(AduroAsyncSocket *)newSocket;

/**
 * Called when a new socket is spawned to handle a connection.  This method should return the run-loop of the
 * thread on which the new socket and its delegate should operate. If omitted, [NSRunLoop currentRunLoop] is used.
**/
- (NSRunLoop *)onSocket:(AduroAsyncSocket *)sock wantsRunLoopForNewSocket:(AduroAsyncSocket *)newSocket;
...
**/
- (BOOL)onSocketWillConnect:(AduroAsyncSocket *)sock;

/**
 * Called when a socket connects and is ready for reading and writing.
 * The host parameter will be an IP address, not a DNS name.
**/
- (void)onSocket:(AduroAsyncSocket *)sock didConnectToHost:(NSString *)host port:(UInt16)port;

/**
 * Called when a socket has completed reading the requested data into memory.
 * Not called if there is an error.
**/
- (void)onSocket:(AduroAsyncSocket *)sock didReadData:(NSData *)data withTag:(long)tag;
...
**/
- (void)onSocket:(AduroAsyncSocket *)sock didReadPartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;

/**
 * Called when a socket has completed writing the requested data. Not called if there is an error.
**/
- (void)onSocket:(AduroAsyncSocket *)sock didWriteDataWithTag:(long)tag;

/**
 * Called when a socket has written some data, but has not yet completed the entire write.
 * It may be used to for things such as updating progress bars.
**/
- (void)onSocket:(AduroAsyncSocket *)sock didWritePartialDataOfLength:(NSUInteger)partialLength tag:(long)tag;
...
**/
- (NSTimeInterval)onSocket:(AduroAsyncSocket *)sock
...
**/
- (void)onSocketDidSecure:(AduroAsyncSocket *)sock;
...
@interface AduroAsyncSocket : NSObject
{
	...
	AduroAsyncReadPacket *theCurrentRead;
	NSTimer *theReadTimer;
	NSMutableData *partialReadBuffer;
	
	NSMutableArray *theWriteQueue;
	AduroAsyncWritePacket *theCurrentWrite;
