...
NSString *const AduroAsyncUdpSocketException = @"AsyncUdpSocketException";
NSString *const AduroAsyncUdpSocketErrorDomain = @"AsyncUdpSocketErrorDomain";
...
@interface AduroAsyncUdpSocket (Private)
...
**/
@interface AduroAsyncSendPacket : NSObject
{
@public
	NSData *buffer;
	NSData *address;
...
@implementation AduroAsyncSendPacket
...
**/
@interface AduroAsyncReceivePacket : NSObject
{
@public
	NSTimeInterval timeout;
	long tag;
...
@implementation AduroAsyncReceivePacket
...
@implementation AduroAsyncUdpSocket
...
- (BOOL)bindToAddress:(NSString *)host port:(UInt16)port error:(NSError **)errPtr
{
	...
	{
		[NSException raise:AduroAsyncUdpSocketException
...
- (BOOL)connectToAddress:(NSData *)remoteAddr error:(NSError **)errPtr
{
	...
	if(errPtr)
	{
		...
		*errPtr = [NSError errorWithDomain:AduroAsyncUdpSocketErrorDomain
...
- (NSError *)getSocketError
{
	...
	return [NSError errorWithDomain:AduroAsyncUdpSocketErrorDomain code:AsyncUdpSocketCFSocketError userInfo:info];
}
...
- (NSError *)getIPv4UnavailableError
{
	...
	return [NSError errorWithDomain:AduroAsyncUdpSocketErrorDomain code:AsyncUdpSocketIPv4Unavailable userInfo:info];
}
...
- (NSError *)getIPv6UnavailableError
{
	...
	return [NSError errorWithDomain:AduroAsyncUdpSocketErrorDomain code:AsyncUdpSocketIPv6Unavailable userInfo:info];
}
...
- (NSError *)getSendTimeoutError
{
	...
	return [NSError errorWithDomain:AduroAsyncUdpSocketErrorDomain code:AsyncUdpSocketSendTimeoutError userInfo:info];
}
- (NSError *)getReceiveTimeoutError
{
	...
	return [NSError errorWithDomain:AduroAsyncUdpSocketErrorDomain code:AsyncUdpSocketReceiveTimeoutError userInfo:info];
}
...
- (BOOL)sendData:(NSData *)data withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	AduroAsyncSendPacket *packet = [[AduroAsyncSendPacket alloc] initWithData:data address:nil timeout:timeout tag:tag];
             ...
             tag:(long)tag
{
	...
	AduroAsyncSendPacket *packet = nil;
	
	if(address4 && theSocket4)
		packet = [[AduroAsyncSendPacket alloc] initWithData:data address:address4 timeout:timeout tag:tag];
	else if(address6 && theSocket6)
		packet = [[AduroAsyncSendPacket alloc] initWithData:data address:address6 timeout:timeout tag:tag];
	else
		return NO;
...
- (BOOL)sendData:(NSData *)data toAddress:(NSData *)remoteAddr withTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	AduroAsyncSendPacket *packet = [[AduroAsyncSendPacket alloc] initWithData:data address:remoteAddr timeout:timeout tag:tag];
...
- (CFSocketRef)socketForPacket:(AduroAsyncSendPacket *)packet
{
	if(!theSocket4)
		return theSocket6;
	if(!theSocket6)
...
- (void)receiveWithTimeout:(NSTimeInterval)timeout tag:(long)tag
{
	...
	AduroAsyncReceivePacket *packet = [[AduroAsyncReceivePacket alloc] initWithTimeout:timeout tag:tag];
...
static void MyCFSocketCallback(CFSocketRef sref, CFSocketCallBackType type, CFDataRef address, const void *pData, void *pInfo)
{
	@autoreleasepool {
		...
		AduroAsyncUdpSocket *theSocket = (__bridge AduroAsyncUdpSocket *)pInfo;
