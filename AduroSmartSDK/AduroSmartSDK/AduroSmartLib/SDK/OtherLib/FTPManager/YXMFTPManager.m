...
#define RunInSeparateThread(...) \
    ...
    [YXMFMThread runInSeparateThread:^{ result = (__VA_ARGS__); }]; \
    result;})

@interface YXMFMThread : NSObject {
@private
...
@implementation YXMFMThread

+ (void)runInSeparateThread:(void (^)(void))block
{
    YXMFMThread *thread = [[YXMFMThread alloc] init];
    thread.block = block;
...
-(BOOL) _ftpActionForServer:(YXMFMServer*)server command:(NSString*)fullCommand;
@end

@implementation YXMFTPManager
...
- (BOOL) _checkFMServer:(YXMFMServer*)server {
    BOOL success = YES;
...
- (BOOL) _uploadData:(NSData *)data withFileName:(NSString *)fileName toServer:(YXMFMServer *)server {
    BOOL success = YES;
...
- (BOOL) _uploadFile:(NSURL*)fileURL toServer:(YXMFMServer*)server {
    BOOL success = YES;
...
- (BOOL) _createNewFolder:(NSString*)folderName atServer:(YXMFMServer*)server {
    BOOL success = YES;
...
- (NSArray*) _contentsOfServer:(YXMFMServer*)server {
    BOOL success = YES;
...
- (BOOL) _downloadFile:(NSString*)fileName toDirectory:(NSURL*)directoryURL fromServer:(YXMFMServer*)server {
    BOOL success = YES;
...
- (BOOL) uploadData:(NSData*)data withFileName:(NSString *)fileName toServer:(YXMFMServer*)server {
...
- (BOOL) uploadFile:(NSURL*)fileURL toServer:(YXMFMServer*)server {
    if (![self _checkFMServer:server]) {
        return NO;
    }
    if (!fileURL) {
...
- (BOOL) createNewFolder:(NSString*)folderName atServer:(YXMFMServer*)server {
    if (![self _checkFMServer:server]) {
        return NO;
    }
    if (!folderName) {
...
- (BOOL) deleteFileNamed:(NSString*)fileName fromServer:(YXMFMServer*)server {
    if (![self _checkFMServer:server]) {
        return NO;
    }
    if (!fileName) {
...
- (BOOL) deleteFile:(NSString *)absolutePath fromServer:(YXMFMServer *)server {
    //this is deprecated.
    //the method may not behave like it used to.
    return RunInSeparateThread([self deleteFileNamed:absolutePath fromServer:server]);
}
- (BOOL) chmodFileNamed:(NSString*)fileName to:(int)mode atServer:(YXMFMServer*)server {
    if (![self _checkFMServer:server]) {
        return NO;
    }
    if (!fileName) {
...
- (NSArray*) contentsOfServer:(YXMFMServer*)server {
    if (![self _checkFMServer:server]) {
        return nil;
    }
    return RunInSeparateThread([self _contentsOfServer:server]);
}
- (BOOL) downloadFile:(NSString*)fileName toDirectory:(NSURL*)directoryURL fromServer:(YXMFMServer*)server {
    if (![self _checkFMServer:server]) {
        return NO;
    }
    if (!fileName) {
...
- (BOOL) checkLogin:(YXMFMServer*)server {
    if (![self _checkFMServer:server]) {
        return NO;
    }
...
+(YXMFMServer*)serverWithDestination:(NSString*)dest username:(NSString*)user password:(NSString*)pass {
    YXMFMServer* server = [[YXMFMServer alloc] init];
    server.destination = dest;
    server.username = user;
    server.password = pass;
    return server;
}
+ (YXMFMServer*) anonymousServerWithDestination:(NSString*)dest {
    YXMFMServer* server = [[YXMFMServer alloc] init];
    server.destination = dest;
    server.username = YXMFTPANONYMOUS;
    server.password = @"";
    return server;
}
@end
